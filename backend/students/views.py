...
def student_registration_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            return JsonResponse(
                {"success": True, "redirect": reverse_lazy("student_course_list")}
            )
        else:
...
def student_enroll_course_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return JsonResponse(
                {
                    "success": True,
                    "redirect": reverse_lazy("student_course_detail", args=[course.id]),
                }
            )
        else:
            return JsonResponse({"success": False, "errors": form.errors}, status=400)
    ...
    return JsonResponse(
        {"form": form.errors}
    )  # For GET request, show empty form errors
